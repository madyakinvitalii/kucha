...
for user in users_list:
    user_cart = User(first_name=user["first_name"], last_name=user["last_name"], age=user["age"], email=user["email"],
                     role=user["role"], phone=user["phone"])
    new_users.append(user_cart)


for order in orders_list:
    order_cart = Order(name=order["name"], description=order["description"], start_date=(datetime.datetime.strptime(order["start_date"], '%m/%d/%Y')), end_date=(datetime.datetime.strptime(order["end_date"], '%m/%d/%Y')),
                     address=order["address"], price=order["price"], customer_id=order["customer_id"], executor_id=order["executor_id"])
    new_orders.append(order_cart)


for offer in offers_list:
    offer_cart = Offer(order_id=offer["order_id"], executor_id=offer["executor_id"])
    new_offers.append(offer_cart)
...
# with app.app_context():
#     query = db.session.query(User.first_name, Offer.executor_id).join(Offer)
...
@app.route("/offers/<int:sid>", methods=["GET"])
def get_offer_by_id(sid: int):
...
def create_user():
    ...
    user = User(first_name=user_json["first_name"], last_name=user_json["last_name"], age=user_json["age"], email=user_json["email"],
                     role=user_json["role"], phone=user_json["phone"])
...
def create_order():
    ...
    order = Order(name=order_json["name"], description=order_json["description"], start_date=(datetime.datetime.strptime(order_json["start_date"], '%m/%d/%Y')), end_date=(datetime.datetime.strptime(order_json["end_date"], '%m/%d/%Y')),
                     address=order_json["address"], price=order_json["price"], customer_id=order_json["customer_id"], executor_id=order_json["executor_id"])
...
def update_order(sid: int):
    ...
    order.start_date = datetime.datetime.strptime(order_json["start_date"], '%m/%d/%Y')
    order.end_date = datetime.datetime.strptime(order_json["end_date"], '%m/%d/%Y')
...
def create_offer():
    ...
    offer = Offer(order_id=offer_json["order_id"], executor_id=offer_json["executor_id"])
...
def update_offer(sid: int):
    offer_json = request.json
    offer = db.session.get(Offer, sid)
    offer.order_id = str(offer_json["order_id"])
    offer.executor_id = str(offer_json["executor_id"])

    db.session.add(offer)
    db.session.commit()

    return "Предложение изменено"
